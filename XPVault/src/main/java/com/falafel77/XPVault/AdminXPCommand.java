...
public class AdminXPCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (subCommand.equals("resetall")) {
            if (args.length == 1) {
                plugin.getXpManager().getCustomConfig().set("", null); // Clear all data
                plugin.getXpManager().saveCustomConfig();
            ...
            long currentSavedXP = plugin.getXpManager().getPlayerSavedXP(targetPlayer);
            long newXP = currentSavedXP;
            ...
            plugin.getXpManager().getCustomConfig().set(targetPlayer.getUniqueId().toString() + ".xp", newXP);
            plugin.getXpManager().saveCustomConfig();
